...
import { Bookmark, BookmarkCheck, TrendingUp, Package } from 'lucide-react';
...
export default function ProductCard({ product, onToggleShortlist, isShortlistPending, showTrendingBadge = true }: ProductCardProps) {
  ...
  return (
    <div
      className="bg-card rounded-2xl shadow-card border border-border/50 overflow-hidden card-hover cursor-pointer"
    ...
    >
      ...
      <div className="p-3">
        ...
        {product.unit && product.unit.trim() && (
          <p className="text-[10px] text-muted-foreground font-body mt-0.5">{product.unit}</p>
        )}
      </div>
    </div>
  );
