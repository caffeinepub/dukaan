...
import { ArrowLeft, Edit, Trash2, TrendingUp, TrendingDown, Bookmark, BookmarkCheck, Package, IndianRupee, Tag, Shield, Ruler } from 'lucide-react';
...
import { useProductById, useDeleteProduct, useSetTrending, useToggleShortlist } from '../hooks/useQueries';
...
export default function ProductDetail() {
  ...
  const handleToggleTrending = async () => {
    if (!product) return;
    try {
      await setTrending.mutateAsync({ productId: id, isTrending: !product.isTrending });
      toast.success(product.isTrending ? 'Removed from trending' : 'Marked as trending');
    } catch {
  ...
  if (error || !product) {
    return (
      <div className="px-4 pt-4">
        <button onClick={() => navigate({ to: '/products' })} className="flex items-center gap-2 text-muted-foreground mb-4">
    ...
    <div className="animate-fade-in">
      ...
      <div className="px-4 pt-4 pb-2 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => navigate({ to: '/products/$productId/edit', params: { productId: productId } })}
          >
            <Edit size={15} />
          </Button>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="icon" className="w-9 h-9 rounded-xl">
                <Trash2 size={15} />
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
      ...
      <div className="px-4 space-y-4 pb-8">
        ...
        <div className="bg-muted/50 rounded-xl p-3 space-y-2">
          <p className="text-xs font-body font-semibold text-muted-foreground uppercase tracking-wider">Price Details</p>
          ...
          {hasWholesale && (
            <div className="flex items-center gap-2 text-sm font-body">
              <span className="font-semibold text-muted-foreground">Wholesale:</span>
              <span className="text-foreground font-semibold">₹{Number(product.wholesalePrice).toLocaleString('en-IN')}</span>
              <span className="text-xs text-muted-foreground">(bulk/trade price)</span>
            </div>
          )}
        ...
        {product.description && (
          <div className="bg-muted/50 rounded-xl p-3">
            <p className="text-xs font-body font-semibold text-muted-foreground uppercase tracking-wider mb-1">Description</p>
        ...
        {(hasAmenities || hasTags || hasWarranty) && (
          <div className="bg-muted/50 rounded-xl p-3 space-y-3">
            ...
            {hasAmenities && (
              <div>
                <p className="text-xs font-body font-semibold text-foreground mb-1.5">Features</p>
                <div className="flex flex-wrap gap-1.5">
                  {product.amenities.map((feature, i) => (
                    <Badge
                      key={i}
            ...
            {hasTags && (
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <p className="text-xs font-body font-semibold text-foreground">Tags</p>
                </div>
                <div className="flex flex-wrap gap-1.5">
            ...
            {hasWarranty && (
              <div className="flex items-start gap-2">
                <Shield size={14} className="text-maroon mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-xs font-body font-semibold text-foreground">Warranty / Shelf Life</p>
                  <p className="text-sm font-body text-muted-foreground">{product.warranty}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            {product.isTrending ? <TrendingDown size={16} /> : <TrendingUp size={16} />}
          ...
          >
            {product.isShortlisted ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}
