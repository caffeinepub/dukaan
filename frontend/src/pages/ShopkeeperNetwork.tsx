...
import { Users, Search, MapPin, Tag, Plus, Store } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="w-12 h-12 gradient-maroon rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
        <span className="font-display font-bold text-ivory text-sm">{initials}</span>
      </div>

      {/* Info */}
      <div className="flex-1 min-w-0">
        <h3 className="font-display font-bold text-sm text-foreground truncate">{shopkeeper.shopName}</h3>
        <p className="text-xs text-muted-foreground font-body truncate">{shopkeeper.name}</p>
...
export default function ShopkeeperNetwork() {
  ...
  const cities = useMemo(() => {
    return Array.from(new Set(shopkeepers.map((s) => s.location))).filter(Boolean).sort();
  }, [shopkeepers]);

  const categories = useMemo(() => {
    return Array.from(new Set(shopkeepers.map((s) => s.category))).filter(Boolean).sort();
  }, [shopkeepers]);
      ...
      <div className="px-4 pt-5 pb-3">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-xl font-bold text-foreground">Shopkeeper Network</h2>
              <p className="text-xs text-muted-foreground font-body">{shopkeepers.length} registered shops</p>
            </div>
          </div>
          <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="font-display font-bold text-foreground mb-1">
              {shopkeepers.length === 0 ? 'No Shopkeepers Yet' : 'No Results Found'}
            </h3>
          ...
          filtered.map((shopkeeper) => (
            <ShopkeeperCard
              ...
              shopkeeper={shopkeeper}
              onClick={() => navigate({ to: '/network/$shopkeeperId', params: { shopkeeperId: shopkeeper.id.toString() } })}
            />
          ))
        )}
      </div>
