...
import type { DashboardMetrics, Product, Shopkeeper } from "../backend";
...
export function useUpdateStockQuantity() {
  ...
  return useMutation({
    mutationFn: async ({
      productId,
      newStock,
    }: { productId: bigint; newStock: bigint }) => {
      if (!actor) throw new Error("Actor not ready");
...
export function useSetTrending() {
  ...
  return useMutation({
    mutationFn: async ({
      productId,
      isTrending,
    }: { productId: bigint; isTrending: boolean }) => {
      if (!actor) throw new Error("Actor not ready");
...
export function useRegisterShopkeeper() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not ready");
      return actor.registerShopkeeper(
        data.name,
        data.shopName,
        data.location,
        data.phone,
