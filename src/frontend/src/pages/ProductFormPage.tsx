...
export default function ProductFormPage() {
  ...
  useEffect(() => {
    if (existingProduct && isEdit) {
      ...
      setMrp(
        Number(existingProduct.mrp) > 0 ? existingProduct.mrp.toString() : "",
      );
      setWholesalePrice(
        Number(existingProduct.wholesalePrice) > 0
          ? existingProduct.wholesalePrice.toString()
          : "",
      );
      setDiscountPercent(
        Number(existingProduct.discountPercent) > 0
          ? existingProduct.discountPercent.toString()
          : "",
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parsedWholesale = BigInt(
      Math.round(Math.max(0, Number(wholesalePrice) || 0)),
    );
    const parsedDiscount = BigInt(
      Math.min(100, Math.max(0, Math.round(Number(discountPercent) || 0))),
    );
    ...
    try {
      ...
      toast.error(
        "Failed to save product. Make sure you are registered as a shopkeeper first.",
      );
    }
  };
      ...
      <form onSubmit={handleSubmit} className="px-4 space-y-4 pb-8">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder={
              form.category
                ? `e.g. ${(PRODUCT_SUGGESTIONS[form.category] ?? [])[0] ?? "Enter product name"}`
                : "Select a category first to get suggestions"
            }
            className="rounded-xl font-body"
          />
          {errors.name && (
          ...
          {/* Product Suggestions */}
          {form.category &&
            (PRODUCT_SUGGESTIONS[form.category] ?? []).length > 0 && (
              <div
                className="space-y-1.5 pt-1"
                data-ocid="product.suggestions.panel"
              >
                <p className="text-[11px] text-muted-foreground font-body flex items-center gap-1">
                  <Sparkles size={10} className="text-amber-500" />
                  Quick select for {form.category}:
                </p>
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide flex-nowrap">
                  {(PRODUCT_SUGGESTIONS[form.category] ?? [])
                    .slice(0, 25)
                    .map((suggestion, index) => {
                      const isSelected = form.name === suggestion;
                      return (
                        <button
                          key={suggestion}
                          type="button"
                          ...
                          className={`flex-shrink-0 px-3 py-1 text-xs rounded-full border font-body transition-all duration-150 whitespace-nowrap cursor-pointer ${
                            isSelected
                              ? "gradient-saffron text-ivory border-transparent shadow-sm"
